...
AS
    ...
    v_id_employe COMMANDES_2.ID_EMPLOYE%TYPE;
BEGIN
    -- 1. Validate inputs
    ...
    SELECT COUNT(*) INTO nc FROM LIGNES_COMMANDES_2
    ...
    IF nc = 0 THEN
        ...
        BEGIN
            ...
            -- Ensure Client
            SELECT COUNT(*) INTO nc FROM CLIENTS_2 WHERE ID_CLIENT = v_cmd.ID_CLIENT;
            IF nc = 0 THEN
                SELECT * INTO v_cl FROM CLIENTS_G@GLOBALE
                WHERE ID_CLIENT = v_cmd.ID_CLIENT;
                INSERT INTO CLIENTS_2 VALUES v_cl;
            END IF;

            -- Ensure Employe
            SELECT COUNT(*) INTO nc FROM EMPLOYES_2 WHERE ID_EMPLOYE = v_cmd.ID_EMPLOYE;
            IF nc = 0 THEN
                SELECT * INTO v_emp FROM EMPLOYES_G@GLOBALE
                WHERE ID_EMPLOYE = v_cmd.ID_EMPLOYE;
                INSERT INTO EMPLOYES_2 VALUES v_emp;
            END IF;

            INSERT INTO COMMANDES_2 VALUES v_cmd;
        END;
    END IF;
    ...
    IF nc = 0 THEN
        ...
        BEGIN
            ...
            -- Ensure Fournisseur
            SELECT COUNT(*) INTO nc FROM FOURNISSEURS_2
            WHERE ID_FOURNISSEUR = v_prod.ID_FOURNISSEUR;
            IF nc = 0 THEN
                SELECT * INTO v_four FROM FOURNISSEURS_G@GLOBALE
                WHERE ID_FOURNISSEUR = v_prod.ID_FOURNISSEUR;
                INSERT INTO FOURNISSEURS_2 VALUES v_four;
            END IF;

            -- Ensure Categorie
            SELECT COUNT(*) INTO nc FROM CATEGORIES_2
            WHERE ID_CATEGORIE = v_prod.ID_CATEGORIE;
            IF nc = 0 THEN
                SELECT * INTO v_cat FROM CATEGORIES_G@GLOBALE
                WHERE ID_CATEGORIE = v_prod.ID_CATEGORIE;
                INSERT INTO CATEGORIES_2 VALUES v_cat;
            END IF;

            INSERT INTO PRODUITS_2 VALUES v_prod;
        END;
    END IF;

    -- 5. Get client ID for the FK
    SELECT ID_CLIENT INTO v_id_client FROM COMMANDES_2
    WHERE ID_COMMANDE = p_id_commande;

    -- 6. Insert the ligne
    INSERT INTO LIGNES_COMMANDES_2
...
AS
    v_id_commande COMMANDES_2.ID_COMMANDE%TYPE;
...
BEGIN
    ...
    IF nc = 0 THEN
        ...
        SELECT COUNT(*) INTO nc FROM COMMANDES_2
...
AS
    nc INTEGER;
    v_old_id_produit PRODUITS_2.ID_PRODUIT%TYPE;
    v_new_id_produit PRODUITS_2.ID_PRODUIT%TYPE;
    v_error_msg VARCHAR2(500);
...
BEGIN
    ...
    IF v_old_id_produit != v_new_id_produit THEN
        ...
        IF nc = 0 THEN
            DELETE FROM PRODUITS_2 WHERE ID_PRODUIT = v_old_id_produit;
        END IF;
    END IF;
