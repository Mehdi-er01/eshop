...
AS
        CURSOR c_commandes(p_id_client COMMANDES_1.ID_CLIENT%TYPE) IS 
        SELECT ID_COMMANDE 
        FROM COMMANDES_1
...
BEGIN
        ...
        FROM CLIENTS_1 CL
        INNER JOIN COMMANDES_1 CM ON CM.ID_CLIENT = CL.ID_CLIENT
        INNER JOIN LIGNES_COMMANDES_1 LC ON LC.ID_COMMANDE = CM.ID_COMMANDE
    ...
    LOOP
                DELETE FROM LIGNES_COMMANDES_1 WHERE ID_COMMANDE = r_commande.ID_COMMANDE;
        
                DELETE FROM COMMANDES_1 WHERE ID_COMMANDE = r_commande.ID_COMMANDE;
    END LOOP;
